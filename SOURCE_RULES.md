...
| Dhan authentication decision | Dhan REST authentication requires `client_id` plus an access token | A generic API key/secret cannot be sent as the access token | `DhanBroker` uses `DHAN_ACCESS_TOKEN`; legacy `DHAN_API_KEY` is accepted only as an alias when its value is itself a valid access token | `DHAN_ACCESS_TOKEN` preferred; `DHAN_API_KEY` alias fallback | ENGINEERING/SAFETY |
| Dhan live diagnostic | Persisted cycle returned Dhan error 808: `Authentication Failed - Client ID or Token invalid` | Account health/market-feed failure must stop the cycle instead of producing fake quotes | Preflight checks account auth and one real market-feed quote before full cycle | Correct client/token pair required | ENGINEERING/SAFETY |
