...
def _classify(title: str, summary: str) -> tuple[str, int, list[str]]:
    ...
    if negative_hits and not positive_hits: sentiment = "NEGATIVE"
    elif positive_hits and not negative_hits: sentiment = "POSITIVE"
    elif positive_hits or negative_hits: sentiment = "MIXED"
...
def fetch_event_news(symbol: str, limit: int = 12) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "material_high_impact": [], "errors": [],
    }
    if not symbol:
        result["errors"].append("NEWS_SYMBOL_EMPTY"); return result
    try:
        import yfinance as yf
    except Exception as exc:
        result["errors"].append(f"YAHOO_FINANCE_UNAVAILABLE: {exc}"); return result
    try:
        raw_items = yf.Ticker(f"{symbol}.NS").news or []
    except Exception as exc:
        result["errors"].append(f"YAHOO_NEWS_REQUEST_FAILED: {exc}"); return result
    now = datetime.now(timezone.utc); normalized: list[dict[str, Any]] = []
    for item in raw_items[: max(1, int(limit))]:
        ...
        age_minutes = None
        if published is not None: age_minutes = max(0.0, (now - published).total_seconds() / 60.0)
        fresh = age_minutes is not None and age_minutes <= 24 * 60
        item_out = {"title": title, "summary": summary, "publisher": publisher or "Yahoo Finance", "url": url,
                    "published_at": published.isoformat() if published else None,
                    "age_minutes": round(age_minutes, 1) if age_minutes is not None else None,
                    "sentiment": sentiment, "materiality": materiality, "matched_terms": terms, "fresh": fresh}
        normalized.append(item_out)
        if fresh and materiality >= 2: result["material_high_impact"].append(item_out)
    ...
    if not normalized and not result["errors"]: result["errors"].append("YAHOO_NEWS_NO_ITEMS")
    return result


def event_news_gate(news: dict[str, Any] | None) -> dict[str, Any]:
    """Convert news evidence into a risk flag, never an entry signal."""
    n = news or {}; items = n.get("items") or []; high = n.get("material_high_impact") or []
    if n.get("source_status") != "AVAILABLE":
        return {"status": "DATA UNAVAILABLE", "action": "NO_NEWS_DECISION", "risk_level": "UNKNOWN",
                "reason": "No verified news evidence available.", "fresh_high_impact_count": 0}
    if not high:
        return {"status": "AVAILABLE", "action": "NO_MATERIAL_EVENT_FLAG", "risk_level": "LOW",
    ...
    return {"status": "AVAILABLE", "action": "REVIEW_BEFORE_ENTRY", "risk_level": risk_level,
