...
def sector(s: str) -> str:
    # Legacy fallback only. Production classification comes from NSE sector
    # membership and is injected through _source_sector().
    groups = {
...
def _telegram_cycle_message(result: dict[str, Any], funds: float | None, sector_info: dict[str, Any]) -> str:
    lines = [
        ...
        f"Candidates: {len(result['candidates'])} | Paper orders: {len(result['orders'])}",
    ...
    if result["candidates"]:
        ...
        for c in result["candidates"][:5]:
            lines.append(
                f"{c['decision']} {c['symbol']} | Sector {c.get('sector','OTHER')} | Entry ₹{float(c.get('entry',0)):.2f} | SL ₹{float(c.get('stop',0)):.2f} | Target ₹{float(c.get('target',0)):.2f} | Qty {int(c.get('quantity',0))} | R:R {float(c.get('rr',0)):.2f} | Score {float(c.get('overall_score',0)):.1f}"
            )
            lines.append(f"Reason: {str(c.get('reason',''))[:260]}")
            if c.get("ai_advisory_text"):
                lines.append(f"AI advisory: {str(c['ai_advisory_text'])[:260]}")
    else:
        lines.append("🔎 TRADE SUGGESTIONS: NONE — no candidate passed every deterministic gate.")
