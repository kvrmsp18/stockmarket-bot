...
class Settings:
    ...
    # IMPORTANT: DHAN_API_KEY is the manually generated long-lived Dhan
    # Access Token entered by the user. No DHAN_ACCESS_TOKEN is read.
    ...
    def dhan_access_token(self) -> str:
        """Compatibility-only empty value; no environment variable is read."""
        return ""

    @property
    def dhan_pin(self) -> str:
        """Automatic Dhan token generation is intentionally disabled."""
        return ""
    ...
    def validate(self) -> None:
        ...
        if self.live_mode_requested and (not self.dhan_client_id or not self.dhan_market_data_token):
            raise ValueError("LIVE requested but manually entered Dhan authentication is unavailable")
