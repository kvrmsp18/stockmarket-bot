...
def scrap_analysis(symbol: str, fundamentals: dict[str, Any] | None) -> ResearchResult:
    ...
    if isinstance(event_risk, dict):
        ...
        if level == "HIGH":
            return ResearchResult(symbol, status="REJECTED", rejection_reason="EVENT_NEWS_HIGH_RISK", metrics={"event_news_risk": event_risk})
        if level == "ELEVATED":
            return ResearchResult(symbol, status="REJECTED", rejection_reason="EVENT_NEWS_REVIEW_REQUIRED", metrics={"event_news_risk": event_risk})
        if level == "UNKNOWN":
            return ResearchResult(symbol, status="REJECTED", rejection_reason="EVENT_NEWS_DATA_UNAVAILABLE", metrics={"event_news_risk": event_risk})
...
def fundamental_score(f: dict[str, Any] | None) -> float:
    ...
    if financial:
        weights = {"profit_growth": 0.30, "eps_growth": 0.15, "roe": 0.25, "earnings_quality": 0.20, "roce": 0.05, "debt_to_equity": 0.05}
    else:
        weights = {"profit_growth": 0.25, "eps_growth": 0.10, "roce": 0.15, "roe": 0.15, "earnings_quality": 0.15, "debt_to_equity": 0.20}
