...
class DhanBroker(BrokerInterface):
    """Read-only Dhan market-data adapter with separate API key and access token."""
    ...
    def _auth_context(self) -> str:
        return (f"credential_source=DHAN_API_KEY_PLUS_ACCESS_TOKEN; "
                f"client_id_configured={'yes' if self.client_id else 'no'}; "
                f"api_key_configured={'yes' if self.api_key else 'no'}; "
                f"access_token_configured={'yes' if self.token else 'no'}; "
                f"access_token_length={len(self.token)}")
    ...
    def _throttle(cls, category: str) -> None:
        ...
        with cls._rate_lock:
            ...
            if category == "quote": cls._last_quote_call = stamp
            else: cls._last_data_call = stamp

    @staticmethod
    def _response_code_and_message(response: requests.Response) -> tuple[str, str]:
        try: payload = response.json()
        except ValueError: return str(response.status_code), response.text[:500]
    ...
    def _request(self, method: str, path: str, category: str = "data", **kwargs: Any) -> Any:
        if not self.client_id or not self.api_key or not self.token:
        ...
        for attempt in range(self._MAX_RETRIES + 1):
            ...
            try:
                ...
                if attempt >= self._MAX_RETRIES: raise RuntimeError(last_error) from exc
                time.sleep(self._RETRY_BASE_SECONDS * (2 ** attempt)); continue
            ...
            if response.status_code == 429 or code == "805":
                ...
                if attempt >= self._MAX_RETRIES: raise RuntimeError(last_error)
                time.sleep(self._RETRY_BASE_SECONDS * (2 ** attempt)); continue
            ...
            try: payload = response.json()
            except ValueError as exc: raise RuntimeError("DHAN_INVALID_JSON_RESPONSE") from exc
    ...
    def _security_id(value: Any) -> int:
        ...
        if not text: raise ValueError("EMPTY_DHAN_SECURITY_ID")
        try: return int(text)
        except (TypeError, ValueError) as exc: raise ValueError(f"INVALID_DHAN_SECURITY_ID: {text[:40]}") from exc
    ...
    def _quote_rows(payload: Any, exchange_segment: str) -> dict[str, dict[str, Any]]:
        ...
        for key, value in rows.items():
            if isinstance(value, dict): out[str(key)] = value
            elif isinstance(value, (int, float)): out[str(key)] = {"last_price": float(value), "ltp": float(value)}
        return out

    def _ltp_batch(self, exchange_segment: str, batch: list[int]) -> dict[str, dict[str, Any]]:
        return self._quote_rows(self._request("POST", "/v2/marketfeed/ltp", category="quote", json={exchange_segment: batch}), exchange_segment)
    ...
    def bulk_quotes(self, instruments: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
        ...
        for item in instruments:
            ...
            try: sid = self._security_id(item.get("security_id", ""))
        ...
        for segment, ids in groups.items():
            ...
            for start in range(0, len(ids), 500):
                ...
                try:
                    ...
                    if not rows: rows = self._ltp_batch(segment, batch)
                except RuntimeError as quote_error:
                    try: rows = self._ltp_batch(segment, batch)
                    except Exception as ltp_error: raise RuntimeError(f"DHAN_MARKETFEED_FAILED: quote={quote_error}; ltp={ltp_error}") from ltp_error
                    if not rows: raise RuntimeError(f"DHAN_MARKETFEED_EMPTY: quote={quote_error}; ltp returned zero rows")
                out.update(rows)
        return out
    ...
    def _history_start_date(today: datetime) -> str:
        ...
        while cursor.weekday() >= 5: cursor -= timedelta(days=1)
        return cursor.isoformat()

    @staticmethod
    def _daily_start_date(today: datetime) -> str: return (today.date() - timedelta(days=420)).isoformat()

    @staticmethod
    def _frame(body: Any) -> pd.DataFrame:
        keys = ["timestamp", "open", "high", "low", "close", "volume"]
        if not isinstance(body, dict) or not all(k in body for k in keys): return pd.DataFrame(columns=keys)
        try:
        ...
        except (TypeError, ValueError, KeyError): return pd.DataFrame(columns=keys)

    def history(self, security_id: str, exchange_segment: str = "NSE_EQ", interval: int = 5, instrument: str = "EQUITY") -> pd.DataFrame:
        now = datetime.now(ZoneInfo("Asia/Kolkata"))
        payload = {"securityId": str(self._security_id(security_id)), "exchangeSegment": str(exchange_segment).strip().upper(), "instrument": str(instrument).strip().upper(), "interval": str(int(interval)), "fromDate": self._history_start_date(now), "toDate": now.date().isoformat()}
    ...
    def daily_history(self, security_id: str, exchange_segment: str = "NSE_EQ", instrument: str = "EQUITY") -> pd.DataFrame:
        now = datetime.now(ZoneInfo("Asia/Kolkata"))
        payload = {"securityId": str(self._security_id(security_id)), "exchangeSegment": str(exchange_segment).strip().upper(), "instrument": str(instrument).strip().upper(), "expiryCode": 0, "oi": False, "fromDate": self._daily_start_date(now), "toDate": now.date().isoformat()}
    ...
    def order(self, symbol: str, side: str, quantity: int, price: float, live: bool = False, **kwargs: Any) -> dict[str, Any]:
        ...
        payload = {"transactionType": side.upper(), "exchangeSegment": kwargs.get("exchange_segment", "NSE_EQ"), "productType": "INTRADAY", "orderType": kwargs.get("order_type", "LIMIT"), "validity": "DAY", "securityId": str(self._security_id(kwargs["security_id"])), "quantity": int(quantity), "price": float(price), "disclosedQuantity": 0, "afterMarketOrder": False}
