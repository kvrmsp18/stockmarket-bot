...
def _portfolio_positions(db: Database, mode: str) -> list[dict]:
    ...
    with db.connect() as con:
        rows = con.execute(
            "SELECT symbol, quantity, entry_price, current_price, mode, payload FROM positions WHERE closed_at IS NULL AND mode=?",
            (mode,),
        ).fetchall()
    output = []
    for row in rows:
        ...
        output.append(
            {
                ...
                "sector": str(payload.get("sector") or "UNKNOWN").strip().upper(),
            }
        )
    return output
...
def fill(db: Database, signal: dict, mode: str = "PAPER") -> str:
    ...
    SCRAP portfolio limits are enforced immediately before the simulated fill using
    only open simulated positions. Manual/real broker holdings are never included.
    """
