...
def heartbeat() -> dict[str, Any]:
    try: return json.loads(HEARTBEAT.read_text(encoding="utf-8")) if HEARTBEAT.exists() else {}
    except Exception: return {}
...
def controls() -> None:
    ...
    hb = heartbeat()
    if hb:
        st.sidebar.caption(f"Worker: {hb.get('state','UNKNOWN')} · {hb.get('updated_at','—')}")
    else:
        st.sidebar.caption("Worker heartbeat: NOT FOUND")
...
def framework_page() -> None:
    ...
    if latest.empty:
        st.info("No persisted framework analyses yet. Run Analysis Now during market hours, or start the always-on worker.")
        return
    symbols=sorted(latest["symbol"].dropna().astype(str).unique()); symbol=st.selectbox("Stock",symbols)
...
def research_page(title: str) -> None:
    header(title, "Persisted research records only. The page never substitutes generic cycle telemetry for missing research data.")
    if title in {"AI Baskets","Basket Detail","Deep Research","Stocks","Stock Screener","360° Stock Analysis"}:
        framework_page(); return
    df=sql("SELECT ts,symbol,payload FROM events WHERE component='research' ORDER BY id DESC LIMIT 500")
    if df.empty: st.warning("DATA UNAVAILABLE: no persisted research records are available for this page."); return
...
def health_page() -> None:
    header("🩺 System Health","Operational state of the always-on worker, market-data pipeline and latest cycle.")
...
def diagnostics_page() -> None:
    ...
    s=status(); st.json(s)
    df=sql("SELECT * FROM events WHERE severity IN ('ERROR','WARN') OR event_type NOT IN ('CYCLE_START','CYCLE_END') ORDER BY id DESC LIMIT 500")
